    ...
    <v-form class="h-full" @submit.prevent="submitForm">
        ...
        <v-form-group class="bg-gray-50 flex items-center justify-end gap-2 text-right border-t border-gray-200">
            <v-content-body class="flex gap-4">
                ...
                <v-button :href="route('panel.states.index')" color="white" :disabled="form.processing">
                    Cancel
                </v-button>
...
export default {
    ...
    methods: {
        ...
        storeRequest() {
            this.form.post(this.route('panel.states.store'), {
        ...
        updateRequest() {
            this.form.patch(this.route('panel.states.update', { state: this.state.data.state_id }), {
        ...
        deleteRequest() {
            this.$alert().confirm(() => {
                return new Promise((resolve, reject) => {
                    this.form.delete(this.route('panel.states.destroy', { state: this.state.data.id }), {
                        onSuccess: () => {
