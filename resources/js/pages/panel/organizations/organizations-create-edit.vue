<template>
    <v-inertia-head :title="(!organization) ? 'Create Organization' : `Editing '${organization.data.name}' organization.`" />
    ...
    <v-form class="h-full" @submit.prevent="submitForm">
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                <template #title>
                    ...
                    <template v-else>
                        Editing '{{ organization.data.name }}' organization.
                    </template>
                </template>

                <template #description>
                    <template v-if="!organization">
                        Fill up the form below to create a new organization for the application.
                    </template>

                    <template v-else>
                        Change and/or modify the details for the organization.
                    </template>
                </template>

                <template #actions>
                    <template v-if="organization && this.$auth().can(['organization.destroy'])">
        ...
        <div class="grid grid-cols-1 divide-y divide-gray-200 gap-12 pb-12">
            <v-form-group class="divide-y divide-gray-200">
                ...
                <v-content-body class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4">
                    ...
                    <v-form-group>
                        <v-form-label>Organization Type</v-form-label>
                        <v-form-select
                            :options="organizationTypes.data.map((organizationType) => ({ label: organizationType.name, value: organizationType.organization_type_id }))"
                            :error="form.errors.organization_type_id"
                            :disabled="form.processing"
                            v-model="form.organization_type_id"
                        />
                        <v-form-error v-if="form.errors.organization_type_id">{{ form.errors.organization_type_id }}</v-form-error>
                    </v-form-group>
        ...
        <v-form-group class="bg-gray-50 flex items-center justify-end gap-2 text-right border-t border-gray-200">
            <v-content-body class="flex gap-4">
                ...
                <v-button :href="route('panel.organizations.index')" color="white" :disabled="form.processing">
                    Cancel
                </v-button>
...
export default {
    ...
    watch: {
        organization: {
            handler(value) {
                ...
                this.form.organization_type_id = value.data.organization_type_id;
    ...
    methods: {
        submitForm() {
            ...
            if (this.organization && this.organization.data && this.organization.data.organization_id) {
                this.updateRequest();
            } else {
                this.storeRequest();
            }
        ...
        storeRequest() {
            this.form.post(this.route('panel.organizations.store'), {
                onSuccess: () => this.$toast().success("Organization created successfully!"),
                onError: (errors) => {
        ...
        updateRequest() {
            this.form.patch(this.route('panel.organizations.update', { organization: this.organization.data.organization_id }), {
                onSuccess: () => this.$toast().success("Organization updated successfully!"),
                onError: (errors) => {
                    Object.keys(errors).forEach((key) => {
                        const error = errors[key];
                        this.$toast().error(error);
        ...
        deleteRequest() {
            this.$alert().confirm(() => {
                return new Promise((resolve, reject) => {
                    this.form.delete(this.route('panel.organizations.destroy', { organization: this.organization.data.organization_id }), {
                        onSuccess: () => {
