    ...
    <div class="h-full flex flex-col">
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                ...
                <template #actions>
                    <template v-if="this.$auth().can(['document-type.store'])">
                        <v-button :href="route('panel.document-types.create')">
        ...
        <v-app-model-table :columns="columns" :data="documentTypes.data" :links="documentTypes.links" :meta="documentTypes.meta">
            <template v-slot:column_document_type_id="{ row: documentType }">
                <v-link :href="route('panel.document-types.show', { document_type: documentType.document_type_id })" class="font-semibold">
                    {{ documentType.name }}
                </v-link>
            </template>
        </v-app-model-table>
