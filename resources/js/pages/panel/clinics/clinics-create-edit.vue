<template>
    <v-inertia-head :title="(!organization) ? 'Create Clinic' : `Editing '${organization.data.name}' organization.`" />
    ...
    <v-form class="h-full" @submit.prevent="submitForm">
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                <template #title>
                    ...
                    <template v-else>
                        Editing '{{ organization.data.name }}' clinic.
                    </template>
                </template>
                ...
                <template #actions>
                    <template v-if="organization && this.$auth().can(['clinic.destroy'])">
        ...
        <v-form-group class="bg-gray-50 flex items-center justify-end gap-2 text-right border-t border-gray-200">
            <v-content-body class="flex gap-4">
                ...
                <v-button :href="route('panel.clinics.index')" color="white" :disabled="form.processing">
                    Cancel
                </v-button>
...
export default {
    ...
    methods: {
        ...
        storeRequest() {
            this.form.post(this.route('panel.clinics.store'), {
        ...
        updateRequest() {
            this.form.patch(this.route('panel.clinics.update', { clinic: this.organization.data.organization_id }), {
        ...
        deleteRequest() {
            this.$alert().confirm(() => {
                return new Promise((resolve, reject) => {
                    this.form.delete(this.route('panel.clinics.destroy', { clinic: this.organization.data.organization_id }), {
                        onSuccess: () => {
