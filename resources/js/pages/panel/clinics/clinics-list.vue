    ...
    <div class="h-full flex flex-col">
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                ...
                <template #actions>
                    <template v-if="this.$auth().can(['clinic.store'])">
                        <v-button :href="route('panel.clinics.create')">
                            Create Clinic
                        </v-button>
                    </template>
                </template>
        ...
        <v-app-model-table :columns="columns" :data="organizations.data" :links="organizations.links" :meta="organizations.meta">
            <template v-slot:column_organization_id="{ row: organization }">
                <v-link :href="route('panel.clinics.show', { clinic: organization.organization_id })" class="font-semibold">
                    {{ organization.name }}
                </v-link>
            </template>
        </v-app-model-table>
