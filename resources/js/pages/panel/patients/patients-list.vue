    ...
    <div class="h-full flex flex-col">
        <v-content-body class="border-b border-gray-200">
            <v-section-heading>
                ...
                <template #description>
                    Manage patients available in the application.
                </template>

                <template #actions>
                    <template v-if="this.$auth().can(['patient.store'])">
                        <v-button :href="route('panel.patients.create')">
                            Create Patient
                        </v-button>
                    </template>
                </template>
        ...
        <v-app-model-table :columns="columns" :data="users.data" :links="users.links" :meta="users.meta">
            <template v-slot:column_user_id="{ row: user }">
                <v-link :href="route('panel.patients.show', { patient: user.user_id })" class="font-semibold">
                    {{ user.name }}
                </v-link>
            </template>
        </v-app-model-table>
